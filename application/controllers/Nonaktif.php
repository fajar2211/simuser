...
//            $data["jenis_petugas"]=$this->Petugas_model->getJenisPetugas();
            $data["nomornonaktif"]=$this->Nonaktif_model->getNomorBaruNonAktif();
            $data["petugasnonaktif"]=$this->Nonaktif_model->getPegawaiPetugasNonAktif();
//            $data["puskesmas"]=$this->Registrasi_model->getPuskesmas();
//            $registrasi->addRegistrasi();
//            $data["status"]=$registrasi->add_registrasi1();            
            $this->load->view('nonaktif',$data);            
	}
    
    public function add_nonaktif(){
...
//                    {
//                    $this->session->set_flashdata('gagal', 'User anda dalam proses permohonan.');
//                    redirect('nonaktif');
//                    } else {
        $nonaktif = $this->Nonaktif_model;
        $data["nomor"]=$this->Nonaktif_model->getNomorBaruNonAktif();
        $nonaktif->addNonaktif();
        $nonaktif->generateNomorBaruNonAktif();
        $this->sendMailNon();
        $this->sendMailAtasanNon();
        redirect('home',$data,'refresh');
//                    }
    }
    
    public function mohonCheck(){
        
    }
    
    public function sendMailNon(){
...
//        echo "<br /><a href='".base_url("registrasi")."'>Kembali ke Form</a>";
    ...
    public function sendMailAtasanNon(){
        $this->load->library('mailer');
//        $email_penerima = $this->input->post('email_penerima');
//        $subjek = $this->input->post('subjek');
//        $pesan = $this->input->post('pesan');
